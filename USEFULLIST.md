# Software List

## Essential
- [Julia 1.0.4 (LTS release)](https://julialang.org/downloads/)  
- [Anaconda (Python 3 version)](https://www.anaconda.com/distribution/)  
- [Git](https://git-scm.com/downloads)  
- [Atom](https://atom.io/)  

## Optional
- [TeXstudio (LaTeX)](https://www.texstudio.org/)  
- [Lyx (LaTeX)](https://www.lyx.org/)  
- [Overleaf (LaTeX)](https://www.overleaf.com/)  
- [Inkscape (image/figure manipulation)](https://inkscape.org/release/inkscape-0.92.4/)  
- [Zotero (reference management)](https://www.zotero.org/)  
- [Mendeley (reference management)](https://www.mendeley.com/download-desktop/)  
- [Dropbox (storage)](https://www.dropbox.com/)  
- [Okular (PDF manipulation)](https://okular.kde.org/download.php)  

## Colors
- [RGB colour generator](https://www.rapidtables.com/web/color/RGB_Color.html#color-table)
- [LaTeX colours](http://latexcolor.com/)
